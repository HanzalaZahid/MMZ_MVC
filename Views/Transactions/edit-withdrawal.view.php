                            ...
                            <select name="intermediate[]" class="intermediate">
                                ...
                                <?php if(isset($beneficiaires) && !empty($beneficiaires)):?>
                                    <?php foreach($beneficiaires as $beneficiairy):?>
                                        ...
                                        <option value="<?php echo $beneficiary_id?>" <?php echo ($beneficiary_id ==  $d['transaction_detail_intermediate_beneficiary'])?"Selected":"";?>><?php echo $beneficiary_name.' ' . ($beneficiary_type  ==  'employee' ? '('.ucfirst($employee_category_name).')' : '('. ucfirst($beneficiary_type) .')').(empty($bank_account_number)?"":" - ".substr($bank_account_number, -6)); ?></option>
                                    <?php endforeach?>
                                <?php endif?>
                            </select>
                        </div>
                        <div class="form_group">
                            ...
                            <select name="destination[]" class="destination">
                                ...
                                <?php if(isset($beneficiaires) && !empty($beneficiaires)):?>
                                    <?php foreach($beneficiaires as $beneficiairy):?>
                                        <?php extract($beneficiairy);?>
                                        <option value="<?php echo $beneficiary_id?>" <?php echo ($beneficiary_id ==  $d['transaction_detail_destination_beneficiary'])?"Selected":"";?>><?php echo $beneficiary_name.' ' . ($beneficiary_type  ==  'employee' ? '('.ucfirst($employee_category_name).')' : '('. ucfirst($beneficiary_type) .')').(empty($bank_account_number)?"":" - ".substr($bank_account_number, -6)); ?></option>
                                    <?php endforeach?>
                                <?php endif?>
                            </select>
                        </div>
